...
if {[info exists env(JG_ENGINE_MODE)] && $env(JG_ENGINE_MODE) ne ""} {
    ...
    set AXI_MASTER_READ_ENGINE_MODE B
}
...
set AXI_MASTER_READ_WRAPPER [file join $CVA5_ROOT formal models axi_master_read_formal_wrapper.sv]
if {[info exists env(AXI_READ_USE_PROVEN_LEMMAS)] && $env(AXI_READ_USE_PROVEN_LEMMAS) eq "1"} {
    puts "AXI_READ_USE_PROVEN_LEMMAS=1: compiling read-wrapper cut assumptions."
    analyze -sv +define+AXI_READ_USE_PROVEN_LEMMAS $AXI_MASTER_READ_WRAPPER
} else {
...
if {[info exists env(AXI_READ_USE_PROVEN_LEMMAS)] && $env(AXI_READ_USE_PROVEN_LEMMAS) eq "1"} {
    assume -enable ${AXI_MASTER_READ_HARNESS}.cut_arvalid_backpressure_implies_requesting_read
    assume -enable ${AXI_MASTER_READ_HARNESS}.cut_requesting_read_holds_arvalid
    puts "Enabled AXI read cut assumptions:"
    puts "  ${AXI_MASTER_READ_HARNESS}.cut_arvalid_backpressure_implies_requesting_read"
    puts "  ${AXI_MASTER_READ_HARNESS}.cut_requesting_read_holds_arvalid"
